...
`adapt-explanation` 是一個適用於 Codex 與 ChatGPT 的開放 agent skill。它會依照讀者的年齡、專業程度、角色、目的、語言、語氣與深度調整說明方式，而不是單純把內容縮短或幼兒化。

[English](README.md) · [Prompt 範例](examples/prompt-gallery.md) · [Skill 指令](skills/adapt-explanation/SKILL.md)
...
```text
$adapt-explanation 請用五歲小朋友能理解的方式解釋 Kubernetes。
```
...
```text
$adapt-explanation 請分別向五歲小朋友、後端初學者和 CTO
解釋 Kubernetes，每個版本不超過 120 字。
```

```text
$adapt-explanation 請把這份 API 文件改寫給剛入職的後端工程師，
所有指令和 JSON 欄位必須保持完全相同。
```

```text
$adapt-explanation 請把這項架構決策分成主管版與資深工程師版。
```
...
主要流程位於 [`SKILL.md`](skills/adapt-explanation/SKILL.md)，受眾設定位於 [`audience-profiles.md`](skills/adapt-explanation/references/audience-profiles.md)。
